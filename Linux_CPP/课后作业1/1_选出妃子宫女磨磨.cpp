...
typedef struct
{
    int age;
    int height;
    string sc;  // 如果想用c语言字符数组的话请将下面的 girl.sc == "xxx"改为 strcmp(girl.sc, "xxx"); 并加上string.h头文件
    bool yz; // 颜值 1漂亮 2一般
} st_girl;
...
int main()
{
        ...
        && (girl.sc == "火辣") && (girl.yz == 1))
    {
        ...
        && ((girl.sc == "火辣") || (girl.sc == "普通")) && (girl.yz == 1))
    {
        cout << "你输入的是宫女" << endl;
    }
    else if (((girl.age >= 35) && (girl.age <= 40))
        && ((girl.height >= 155) && (girl.height <= 165))
        && ((girl.sc == "普通") || (girl.sc == "飞机场")) && (girl.yz == 0))
    {
        cout << "你输入的是磨磨" << endl;
    }
    else cout << "error please try agin\n";
    }
}
